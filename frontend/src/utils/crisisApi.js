...
export class CrisisKeywordDetector {
    ...
    // 拆字和变形映射（增强版）
    static VARIANT_MAP = {
        "自杀": ["zi sha", "自 杀", "自_杀", "自-杀", "自*杀", "zi4", "zisha", "自s", "自4"],
        "想死": ["想 死", "想4", "想si", "xiang死", "想sǐ", "xiang4", "想 4"],
        "绝望": ["绝 望", "jue望", "绝wang", "jué望", "绝 wang"],
        "崩溃": ["崩 溃", "beng溃", "崩kui", "běng溃", "崩 kui"],
        "痛苦": ["痛 苦", "tong苦", "痛ku", "tòng苦", "痛 ku"],
        "不想活": ["不想 活", "不 想活", "buxianghuo", "不想huo"],
    ...
    initSimilarityMaps() {
        ...
        // 扩展原有关键词
        for (const [category, keywords] of Object.entries(CrisisKeywordDetector.CRISIS_KEYWORDS)) {
    ...
    addHomophoneVariants(keyword) {
        for (const [char, variants] of Object.entries(CrisisKeywordDetector.HOMOPHONE_MAP)) {
        ...
        for (const [category, keywords] of Object.entries(CrisisKeywordDetector.CRISIS_KEYWORDS)) {
            ...
            for (const keyword of keywords) {
                ...
                if (matches.length > 0) {
                    ...
                    categoryScore += matches.length * this.getKeywordWeight(keyword, category);
                }
            }
    ...
    findMatches(normalizedText, keyword) {
        ...
        for (const expandedKeyword of this.expandedKeywords) {
            if (expandedKeyword !== keyword && normalizedText.includes(expandedKeyword)) {
...
export class CrisisUtils {
    ...
    static getWarningConfig(level) {
        const configs = {
            ...
            'critical': {
                title: '紧急危机状态',
                message: '检测到严重的心理危机信号！请立即寻求紧急心理危机干预帮助。您的生命很珍贵！',
                hint: '立即寻求紧急帮助！',
                color: '#f5222d',
                icon: '🆘',
                priority: 4,
    ...
    static showPsychologicalHotlines() {
        ...
        uni.showActionSheet({
            ...
            itemList: hotlines,
            success: (res) => {
                const phoneNumbers = ['400-161-9995', '400-161-9995', '021-34289888', '020-81899120'];
