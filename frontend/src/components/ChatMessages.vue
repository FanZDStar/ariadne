    ...
    >
      ...
      >
        ...
        <view :class="['message', message.role]">
          ...
          <!-- 图片内容 -->
          <view
            v-if="message.img_urls && message.img_urls.length > 0"
            class="message-images"
          >
            <image
              v-for="(imgUrl, imgIndex) in message.img_urls"
              :key="imgIndex"
              :src="imgUrl"
              mode="aspectFill"
              class="message-image"
              @click="previewImage(imgUrl, message.img_urls)"
            />
          </view>
          <!-- 为AI消息添加光标闪烁效果 -->
          <text
            v-if="
              message.role === 'assistant' && isTyping(message.content, index)
            "
            class="cursor"
            >|</text
          >
    ...
    handleNewMessages(newMessages) {
      ...
      newMessages.forEach((message, index) => {
        ...
        // AI消息使用打字机效果
          ...
          if (
            !message.content ||
            this.displayedMessages[index].content === message.content
          ) {
            this.displayedMessages[index] = { ...message };
            if (this.isAutoScroll) {
              this.scrollToBottom();
