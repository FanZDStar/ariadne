        ...
        <view v-if="currentStage === 'training'" class="training-stage">
            ...
            <view class="scenario-content">
                <view class="scenario-background">
                    ...
                    <text class="scenario-description">{{ currentTrainingStep.description }}</text>
                </view>

                <view class="scenario-dialogue">
                    <view v-for="message in currentTrainingStep.dialogue" :key="message.id" class="dialogue-item"
                        :class="{ 'risk-message': message.isRisk, 'normal-message': !message.isRisk }">
                ...
                <view class="question-section">
                    <text class="question-title">{{ currentTrainingStep.question.title }}</text>
                    <text class="question-text">{{ currentTrainingStep.question.text }}</text>

                    <view class="answer-options">
                        <view v-for="option in currentTrainingStep.question.options" :key="option.id"
                            class="option-item" @click="selectAnswer(option)">
                            <view class="option-header">
                                <text class="option-letter">{{ String.fromCharCode(65 + option.id - 1) }}</text>
        ...
        <view v-if="currentStage === 'feedback'" class="feedback-stage">
            <view class="feedback-card">
                <view class="feedback-header">
                    <text class="feedback-icon">{{ feedbackResult.isCorrect ? '✅' : '❌' }}</text>
                    <text class="feedback-title">{{ feedbackResult.isCorrect ? '回答正确！' : '需要改进' }}</text>
                ...
                <view class="feedback-content">
                    <view class="correct-analysis">
                        ...
                        <text class="analysis-text">{{ feedbackResult.correctAnalysis }}</text>
                    </view>

                    <view class="risk-explanation">
                        <text class="explanation-title">风险解释：</text>
                        <text class="explanation-text">{{ feedbackResult.riskExplanation }}</text>
                    </view>
                ...
                <view class="feedback-actions">
                    <view class="feedback-btn primary" @click="nextScenario">
                        <text class="btn-text">{{ currentScenario < totalScenarios ? '下一个场景' : '查看结果' }}</text>
                    </view>
                </view>
            </view>
        </view>
            ...
            <view class="result-overview">
                <view class="final-score">
                    ...
                    <view class="score-level" :class="getScoreLevel(finalResult.totalScore)">
                        <text class="level-text">{{ getScoreLevelText(finalResult.totalScore) }}</text>
                    </view>
                </view>
        ...
        return {
            ...
            trainingModes: [
                ...
                },
                {
                    ...
                    title: '网络安全防护',
                    icon: '🌐',
                    description: '识别网络交往中的风险和陷阱',
                    level: 1,
                    scenarios: 6,
                    duration: '15-20分钟',
                    skills: ['网络识别', '信息保护', '安全交流'],
                    objectives: [
                        '识别网络交友中的风险信号',
                        '保护个人隐私和信息安全',
                        '学会安全的网络交流方式',
                    ...
                    riskSignals: [
                        {
                            type: '信息收集',
                            icon: '🔍',
                            description: '过度询问个人信息、财务状况等隐私'
                        },
                        {
                            type: '快速发展',
                            icon: '⚡',
                            description: '异常快速地发展关系，表达强烈情感'
                        },
                        {
                            type: '金钱要求',
                            icon: '💰',
                            description: '以各种理由要求转账、借钱或投资'
                        }
                    ],
                    strategies: [
                        '保护个人隐私信息',
                ...
                },
                {
                    ...
                    title: '职场骚扰应对',
                    icon: '🏢',
                    description: '应对职场中的不当行为和骚扰',
                    level: 3,
                    scenarios: 10,
                    duration: '25-35分钟',
                    skills: ['骚扰识别', '证据收集', '投诉维权'],
                    objectives: [
                        '识别职场骚扰的各种形式',
                        '学会收集和保存证据',
                        '掌握投诉和维权渠道',
                    ...
                    riskSignals: [
                        {
                            type: '权力滥用',
                            icon: '👑',
                            description: '利用职位优势进行不当要求或威胁'
                        },
                        {
                            type: '性骚扰',
                            icon: '🚫',
                            description: '不当的身体接触或性暗示言论'
                        },
                        {
                            type: '职场霸凌',
                            icon: '👊',
                            description: '持续的恶意排挤、辱骂或刁难'
                        }
                    ],
                    strategies: [
                        '明确拒绝不当行为',
    ...
    methods: {
        ...
        selectAnswer(option) {
            ...
            this.feedbackResult = {
                isCorrect,
                scoreGained,
                correctAnalysis: isCorrect ? '你正确识别了这个风险信号！' : '这个答案需要再考虑一下。',
                riskExplanation: '这是典型的情感操控行为，通过威胁分手来控制对方的社交关系。',
                protectionAdvice: [
                    '明确表达自己的立场和边界',
                    '不被情感威胁所绑架',
                    '寻求朋友或专业人士的意见',
        ...
        nextScenario() {
            if (this.currentScenario < this.totalScenarios) {
        ...
        getHint() {
            uni.showModal({
                title: '💡 提示',
                content: '注意观察对方是否试图通过威胁、情感绑架等方式控制你的行为。健康的关系应该基于相互尊重。',
                showCancel: false,
                confirmText: '知道了'
            });
        },
