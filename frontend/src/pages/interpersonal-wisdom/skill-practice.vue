...
export default {
    ...
    methods: {
        ...
        async sendMessage() {
            ...
            try {
                // 检查是否是第一条用户消息（除了欢迎消息）
                const userMessagesCount = this.chatMessages.filter(msg => msg.role === 'user').length;
                ...
                if (response.statusCode === 200) {
                    ...
                    if (result.response_type === 'roleplay') {
                        ...
                        if (result.practice_completed) {
                            this.practiceCompleted = true;
                            this.practiceResult = {
                                feedback: `很棒的练习！你在"${this.skillInfo?.title || this.skillInfo?.name}"的情景对话中表现出色。`,
                ...
                console.error('发送消息失败:', error);
                // 使用备用角色扮演响应
                this.handleFallbackRoleplayResponse(userMessage, this.chatMessages.filter(msg => msg.role === 'user').length === 1);
            } finally {
                this.isAiTyping = false;
            }
        },
        ...
        handleFallbackRoleplayResponse(userMessage, isFirstMessage) {
            ...
            setTimeout(() => {
                ...
                if (Math.random() < 0.3 && this.chatMessages.length > 8) {
                    this.practiceCompleted = true;
                    this.practiceResult = {
                        feedback: `很棒的情景对话练习！你在"${this.skillInfo?.title || this.skillInfo?.name}"中展现了良好的沟通技巧。`,
                        score: Math.floor(Math.random() * 20) + 80,
                        improvements: [
                            '在真实情况下运用这些技巧',
                            '继续观察和回应对方的情感',
        ...
        handleFallbackResponse(userMessage) {
            ...
            setTimeout(() => {
                ...
                if (Math.random() < 0.3 && this.chatMessages.length > 6) {
                    this.practiceCompleted = true;
                    this.practiceResult = {
                        feedback: `太棒了！你在"${this.skillInfo?.title || this.skillInfo?.name}"的练习中表现出色。继续在实际生活中运用这些技巧，会让你的人际关系更加和谐！`,
